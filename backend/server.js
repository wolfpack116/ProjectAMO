...
app.post('/api/route-briefing', (req, res) => {
  ...
  try {
    ...
    try {
      const model = buildRouteModelFields(body.routeGeometry, body)
...
// 경로의 KIM/KTG 단면 필드를 로드한다(Phase 2b enroute 모델 요약용).
// NOTE: /api/briefing/cross-section 라우트와 로딩 로직이 겹친다 — 추후 공유 헬퍼로 dedupe 대상.
function buildRouteModelFields(routeGeometry, body = {}) {
  const latest = readKimNwpLatest(DATA_ROOT)
