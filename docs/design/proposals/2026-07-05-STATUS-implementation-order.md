...
| | #6 TAF 일원화 | ⬜ **다음(새 세션)** | ⚠️ 무동작 리팩터 아님 — 판정 불일치 실재(RKSI 패널 IFR vs 브리핑 MVFR). **결정: 패널식 3단계+공항미니마로 통일**(브리핑 라벨 바뀜→before/after 육안 필수). 함정=런타임경계·미니마 출처·MVFR폐기 파급. 상세 [#6 스펙](2026-07-05-remaining-features-implementation-notes.md) 상단 결정블록 |
...
## 다음 액션
→ #1 완료(라이브 육안까지, 커밋 `ad41ba0`). #4 보류(cutoff 실제값 대기). #6 **도메인 결정 완료**(패널식 3단계+미니마로 통일) — 실제 리팩터는 **새 세션**에서: 라벨 바뀌는 큰 변경이라 회귀·경계 함정 있음. 착수 시 [#6 스펙](2026-07-05-remaining-features-implementation-notes.md) 상단 "2026-07-05 결정블록" 먼저 읽기.
- #4 재개 조건: KMA KIM 사이클별 `data_cutoff` 실제값 확인.
- #6 착수 전: 백엔드 미니마 출처(기본 vs 클라이언트 전달) 한 가지 더 결정.
