...
## 마무리 A 리뷰 — 접근성(AA) 패스 (2026-06-27, design-critique + accessibility-review)
> 측정: preview에서 상대휘도/대비 비율·터치타깃 px 직접 산출. 데스크톱 1280 브리핑 + 모바일 390 위저드/시트.
- **대비(1.4.3) 해결:**
  - `#94a3b8`(slate-400)을 본문/라벨에 사용 → `.bv-table th`·`.bv-airport-role`(2.46:1), `.bv-muted` ①/④ takeaway(2.37:1) **실패**. `#6b7280`(라벨, 4.64:1)·`#4b5563`(takeaway, 틴트 위 통과+위계↑)로 보정.
  - 비활성 세그먼트/스텝 라벨 `#64748b`/`#eef2f7` 4.23:1 → `#475569`(5.17:1). `.route-type-seg`·`.rb-step`.
  - VFR 녹 배지/보드칩 `#15803d`/녹틴트 4.18:1 → `#166534`(5.94:1). `.bv-cat-vfr`·`.bv-chip.bv-green`. 점은 currentColor라 함께 진해짐.
- **터치타깃(2.5.5) 해결(≥44px, 모바일 스코프):** 시트 닫기 32→44, 위저드 스텝탭 36→44, route-type 세그 34→44, 이전/다음 40→44, 출발/도착/교체 select·picker 36→44, 스왑 36→44. `.rb-mobile`/시트 한정 → 데스크톱 밀도 불변.
- **검증:** build 성공 · 데스크톱 스모크 `"ok": true`(단면도·nav·리본 불변) · 캡처 재기록.
- **남은(경계/저우선):** 인라인 단면도 결과 컨트롤(`연직단면도 생성/열기`·순항고도 ft input) 모바일 34~36px — 리본↔단면도 x정렬(6.04%/2.71%) 회귀 민감 영역이라 보류, 별도 검토. 데스크톱 nav 비활성 `③ 현재` 4.08:1(틴트 위)도 동일 팔레트라 함께 검토 가능.
